...
#include "InterferingPeakFit.h"

using namespace std;

InterferingPeakFit::InterferingPeakFit() {
  params = NULL;
  nfits = 0;
  file = NULL;
  c1 = new TCanvas();
 ...
 */
bool InterferingPeakFit::getHisto(string filename, string histoname) {
  ...
  if(file->GetListOfKeys()->Contains(histoname.c_str())) {
    ...
    cout<<"Error in InterferingPeakFit::GetHisto!! Either the file isnt valid or the histogram name "
 ...
 */
void InterferingPeakFit::getRanges() {
 ...
 * full function
 */
void InterferingPeakFit::createFullFunction() {
  string fullname = "";
 ...
 */
void InterferingPeakFit::fitIndividuals() {
...
void InterferingPeakFit::fitFull() {
 ...
 */
bool InterferingPeakFit::drawFit() {
...
void InterferingPeakFit::saveResults(string filename) {
