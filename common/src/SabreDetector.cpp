/*
  ...
  >> wedges are 0 -- 7 moving counterclockwise (i.e. pos phi)
...
void SabreDetector::CalculateCorners() {
	...
	for(int i=0; i<m_nWedges; i++) {
	        x0 = m_Router * std::cos(-m_deltaPhi_flat/2.0 + (i+1)*m_deltaPhi_flat_wedge);
		y0 = m_Router * std::sin(-m_deltaPhi_flat/2.0 + (i+1)*m_deltaPhi_flat_wedge);
		z0 = 0.0;
		m_wedgeCoords_flat[i][0].SetVectorCartesian(x0, y0, z0);

		x1 = m_Router * std::cos(-m_deltaPhi_flat/2.0 + i*m_deltaPhi_flat_wedge);
		y1 = m_Router * std::sin(-m_deltaPhi_flat/2.0 + i*m_deltaPhi_flat_wedge);
		z1 = 0.0;
		m_wedgeCoords_flat[i][1].SetVectorCartesian(x1, y1, z1);

		x2 = m_Rinner * std::cos(-m_deltaPhi_flat/2.0 + i*m_deltaPhi_flat_wedge);
		y2 = m_Rinner * std::sin(-m_deltaPhi_flat/2.0 + i*m_deltaPhi_flat_wedge);
		z2 = 0.0;
		m_wedgeCoords_flat[i][2].SetVectorCartesian(x2, y2, z2);

		x3 = m_Rinner * std::cos(-m_deltaPhi_flat/2.0 + (i+1)*m_deltaPhi_flat_wedge);
		y3 = m_Rinner * std::sin(-m_deltaPhi_flat/2.0 + (i+1)*m_deltaPhi_flat_wedge);
		z3 = 0.0;
...
Mask::Vec3 SabreDetector::GetHitCoordinates(int ringch, int wedgech) {
	...
	double z = 0;
	*/
	//kgh add--20201214: randomize within pixel	
	double randspot_r = (rand()%100)/100.; //number btwn 0 and 1 in 100 increments
	double randspot_phi = (rand()%100)/100.;
	double rspot = m_Rinner + m_deltaR_flat_ring*(ringch+randspot_r);
	double phi_spot = -m_deltaPhi_flat/2.0 + (wedgech+randspot_phi)*m_deltaPhi_flat_wedge;
	double x = rspot*cos(phi_spot);
	double y = rspot*sin(phi_spot);
	double z = 0;	
...
double SabreDetector::GetHitTheta(int ringch, int wedgech) {

  Mask::Vec3 hit = GetHitCoordinates(ringch,wedgech);
  return hit.GetTheta();

}

double SabreDetector::GetHitPhi(int ringch, int wedgech) {
