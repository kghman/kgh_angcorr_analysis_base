			     ...
			     int lam1, int lam, int lam2)
{
  ...
  for (q=0.;q<=lamstr;q+=1.){
    sum+= (2.-d(q,0.)) * clebsch((double)lam1,(double)lam,(double)lam2,
      ...
      legen(lam,(int)q,cos(theta1)) * legen(lam2,(int)q,cos(theta2)) * 
      cos (q*phi);
  }
  return(sum);
}
...
double spherical_harmonic(int lam, int q, double theta, double phi)
{
  ...
  value=1./sqrt(4.*M_PI);
  if (q>=0){
    value *= (minuseinshoch(q) * cos(q*phi));
  }
  else {
    value *= cos(q*phi);
  }
  value *= sqrt((2*lam+1)*fakul(lam-abs(q))/fakul(lam+abs(q)));
  return(value*legen(lam,abs(q),cos(theta)));
}

double spherical_harmonic_cosine(int lam, int q, double cos_theta, double phi)
{
  double value;
  value=1./sqrt(4.*M_PI);
  if (q>=0){
  ...
  value *= sqrt((2*lam+1)*fakul(lam-abs(q))/fakul(lam+abs(q)));
  return(value*legen(lam,abs(q),cos_theta));
}
...
complex<double> complex_spherical_harmonic(int lam,int q,double theta,double phi)
{
  ...
  if (abs(q)>lam)
    return(0);
  cvalue = exp((complex<double>)(1i*(phi*q)))/sqrt(4.*M_PI);
  if (q>=0){
    cvalue *= minuseinshoch(q);
  }
  cvalue *= sqrt((2*lam+1)*fakul(lam-abs(q))/fakul(lam+abs(q)));
  cvalue *= gsl_sf_legendre_Plm (lam,abs(q),cos(theta));
  return(cvalue);
}

complex<double> complex_spherical_harmonic_cosine(int lam,int q,double cos_theta,double phi)
{
  complex<double> cvalue;
  ...
  cvalue *= sqrt((2*lam+1)*fakul(lam-abs(q))/fakul(lam+abs(q)));
  cvalue *= gsl_sf_legendre_Plm (lam,abs(q),cos_theta);
  return(cvalue);
}
...
double wiko_triple_angular_function(double theta1, double theta2, double theta3,
				    ...
				    int lam1,int lam2,int lam3,int lam4,int lam5)
{
  int q1,q2,q3;
  double sum=0.,fak=0.;
  for (q1=0;q1<=wmin(lam2,lam3);q1++){
...
double wiko_orient_gaussian(int new_spins,double i1,double sigma,int lam)
{
  double m;
  int l;
  double sum=0.,norm=0.;
...
double wiko_orient_single_m(int new_spins,double i1,double them,int lam)
{
  int l;
  double m;
  static double *c[MAX_LAMBDA_INDEX/2+1];
...
double wiko_dumb_orient_gaussian(int new_spins,double i1,double sigma,int lam)
{
  double m;
  double sum=0.,norm=0.;
...
double wiko_not_oriented(int new_spins,double i1,double sigma,int lam)
{
  if (lam==0)
    return(1.);
  else
...
double wiko_f3(int l1,int l2,double i1,double i2,int lam1,int lam,int lam2)
{
  if (lam2!=0){
...
double wiko_alpha_f3(int l1,int l2,double i1,double i2,int lam1,int lam,int lam2)
{
  double r1,r2,r3,r4;
...
double wiko_prot_f1(double j1,double j2,double i1,double i2,int lam)
{  
...
int wiko_init_double_setup(wiko_setup *setup,
			   double (*orient)(int,double,double,int),
			   double (*f_1)(int,int,double,double,int,int,int),
			   double (*f_2)(int,int,double,double,int,int,int)
			   )
{
...
int wiko_init_triple_setup(wiko_setup *setup,
			   ...
			   double (*f_1)(int,int,double,double,int,int,int),
			   double (*f_2)(int,int,double,double,int,int,int),
			   double (*f_3)(int,int,double,double,int,int,int)
			   )
{
  setup->orient_function=orient;
  setup->fold=3;
...
double wiko_double_corr(wiko_setup *setup,
			...
			double sigma,double delta1,double delta2,
			int k1,int k,int k2)
{
  ...
  l1a=wmax(1,(int)(fabs(i2-i1)));
  l1b=l1a+1;
  l2a=wmax(1,(int)(fabs(i4-i3)));
  l2b=l2a+1;
  max_lambda = 2* wmax(i1,wmax(l1b,l2b));
  if (new_spins){
...
double wiko_triple_corr(wiko_setup *setup,
			...
			double sigma,double delta1,double delta2,double delta3,
...
double wiko_dis_from_oriented(int new_spins,double i1,double i2,
			      double sigma,double delta,int k)
{
  double a,b;
  int la,lb;

  la=wmax(1,(int)(fabs(i2-i1)));
  lb=la+1;
...
double wiko_deorient(int new_spins,double i1,double i2,double delta,int lambda)
{
...
double gg_partial_angular_function(double theta1,double phi1,
				   double theta2,double phi2,
				   int lam0,int lam1,int lam2,int q0)
{
  int q1,q2;
  double sum=0.;
  ...
  for (q2= -lam2;q2<=lam2;q2++){
	...
	(cos(q1*phi1)*cos(q2*phi2)
	 +sin(q1*phi1)*sin(q2*phi2));
    }
  }
  sum *= (1./sqrt(2*lam2+1.));
  return(sum);
...
complex<double> partial_g_corr_complex(int new_spins,
				       int l0,int q0,
				       double i2,double i3,
				       int l1,int l2,double delta,
				       double theta1,double phi1)
{
  ...
  sum =
    (1./sqrt(2*lam0+1))*
    f1[lam0/2]*
    complex_spherical_harmonic(lam0,q0,theta1,phi1);
  return(sum);
}
...
double partial_g_corr_3mult(int new_spins,
			    ...
			    double theta1,double phi1)
{
  //now includes lowest THREE multipolarities, e.g. E1, M2, E3
  // delta21 is ratio M2/E1
  // delta31 is ratio E3/E1
  int lam0;
  double sum=0;
  static double f1[10];

  if (new_spins){
    for (lam0=0;lam0<=2*l3;lam0+=2){
      f1[lam0/2]=(wiko_f3(l1,l1,i2,i3,lam0,lam0,0) +
		  2*delta21*wiko_f3(l1,l2,i2,i3,lam0,lam0,0) +
		  delta21*delta21*wiko_f3(l2,l2,i2,i3,lam0,lam0,0) +
		  2*delta31*wiko_f3(l1,l3,i2,i3,lam0,lam0,0) +
		  delta31*delta31*wiko_f3(l3,l3,i2,i3,lam0,lam0,0) +
		  2*delta21*delta31*wiko_f3(l2,l3,i2,i3,lam0,lam0,0));
      f1[lam0/2] /= (1 + delta21*delta21 + delta31*delta31);
    }
  }
  lam0=l0;
  // This normalization was changed in accordance with the Hamilton
...
complex<double> partial_g_corr_3mult_complex(int new_spins,
					     int l0,int q0,
					     double i2,double i3,
					     int l1,int l2,int l3,
					     double delta21, double delta31,
...
complex<double> partial_p_corr_complex(int new_spins,
				       int l0,int q0,
				       double i2,double i3,
				       double theta1,double phi1)
{
					...
					int k,int q)
{
  ...
  for (i=0;i<1;i++){
    ...
    for (mba=-lba;mba<=lba;mba++){
      mbb=mba-q;
      if ((abs(mbb)<=wmin(lbb,i3))&&(abs(mba)<=wmin(lba,i3))){
	...
	value *= conj((complex<double>)(spherical_harmonic(lbb,-mbb,theta2,phi)));
				      ...
				      int k,int q)
{
  ...
  for (i=0;i<1;i++){
    ...
    for (mba=-lba;mba<=lba;mba++){
      mbb=mba-q;
      if ((abs(mbb)<=wmin(lbb,i2))&&(abs(mba)<=wmin(lba,i2))){
...
double legen(int k, int q, double costet)
{
  return gsl_sf_legendre_Plm (k,q,costet);
}

double clebsch(double i1,double i2,double i,double m1,double m2,double m)
{
  double r;
  r = minuseinshoch(m+i1-i2)*sqrt(2*i+1) //Brink & Satchler for conversion factor
...
double wign3j(double a,double b,double c,double xx,double yy,double zz)
{
  double r;
...
double wign6j(double a,double b,double c,double xx,double yy,double zz)
{
  double r;
...
double wign9j(double a, double b, double c, double xx, double yy, double zz, double gg, double hh, double pp)
{
  double r;
