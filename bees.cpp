/*
  ...
  bees.cpp: generates orientation tensors Bkq from fortran output fort.37 file
	    ...
	    ASSUMPTIONS:
	    * spin 1/2 particle decay (proton or neutron) from a SINGLE state
...
int main(int argc, char* argv[]) {
  ...
  double Ji, Jf; //initial and final spin of decay transition (Jf=0)
  int    Pi, Pf; //corresponding parities (+ or - 1) (Pf=+1)
  ...
  Ji = Bkq.J();
  Jf = 0.0; //***one of the MAIN ASSUMPTIONS
  Pf = +1;
  ...
  WikoParticleClass Wiko(Pi*Ji,BR);
  ...
  cout << "READING FROM FILE: " << fort37_name << endl
       ...
       << ">> Fixed lab angle = " << rxnang_lab*RADTODEG << " deg (rxncm " << rxnang_rxncm*RADTODEG << " deg, index " << rxn_angle_index << ": " << Bkq.Angle(rxn_angle_index)*RADTODEG << " deg)" << endl
       ...
       << ">> transition from Ji = " << Ji << Pi_char << " to Jf = " << Jf << Pf_char << " (L = " << Wiko.Get_L() << ", BR = " << BR << ")" << endl
  ...
  for (int k=0; k<=k_max; k++) {
	...
	for (int xyang=0; xyang<NUM_XY_ANGS; xyang++) { //relative azimuthal angle
      ...
      k_fill = k;
      q_fill = q;
      akq_real_fill = real(Wiko.Decay_Coefficient(k)*Bkq.Bkq_Complex(k,q,rxn_angle_index));
      akq_imag_fill = imag(Wiko.Decay_Coefficient(k)*Bkq.Bkq_Complex(k,q,rxn_angle_index));
      coeff_tree.Fill();
